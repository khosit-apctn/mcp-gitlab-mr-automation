...
export const TOOLS = {
  ...
  get_mr_details: {
    name: 'get_mr_details',
    description: 'Get Merge Request description and diff content',
    schema: z.object({
      cwd: z.string().describe('The current working directory of the project'),
      mrIid: z.number().describe('The internal ID (iid) of the merge request')
    }),
    handler: async (args: any) => {
      const data = await getMrDetails(args.cwd, args.mrIid);
      return {
        content: [{ type: 'text', text: JSON.stringify(data, null, 2) }]
      };
    }
